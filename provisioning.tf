...
data "template_file" "dockercfg_registry_config" {
  ...
  vars {
    ...
    auth  = "${element( split(",", lookup( var.docker_registry_config, element(keys(var.docker_registry_config), count.index) ) ), 0 ) }"
    email = "${element( split(",", lookup( var.docker_registry_config, element(keys(var.docker_registry_config), count.index) ) ), 1 ) }"
  }
}
...
data "template_file" "docker_registry_config" {
  ...
  vars {
    repo     = "${element(keys(var.docker_registry_config), count.index)}"
    username = "${element( split(",", lookup( var.docker_registry_config, element(keys(var.docker_registry_config), count.index) ) ), 0 ) }"
    password = "${element( split(",", lookup( var.docker_registry_config, element(keys(var.docker_registry_config), count.index) ) ), 1 ) }"
    email    = "${element( split(",", lookup( var.docker_registry_config, element(keys(var.docker_registry_config), count.index) ) ), 2 ) }"
  }
}
